...
##### Loading Data #####
songs = read.csv("FINAL_DATA_2.csv", stringsAsFactors=FALSE)
...
corpus_lyrics = Corpus(VectorSource(songs_transformed$Lyrics))
...
sparse_lyrics = removeSparseTerms(frq_lyrics, 0.90)
sparse_lyrics

lyric_words = as.data.frame(as.matrix(sparse_lyrics))
colnames(lyric_words) = make.names(colnames(lyric_words))

songs_w_nlp = cbind(songs_transformed, lyric_words)
songs_w_nlp$Lyrics = NULL
# write.csv(songs_w_nlp, 'all_data_w_features_nlp.csv')
...
tableAccuracy(test$Top100, predict_log > 0.5) # 0.8074844
tableTPR(test$Top100, predict_log > 0.5) # 0.4976378
tableFPR(test$Top100, predict_log > 0.5) # 0.08135593
...
tableAccuracy(test$Top100, predict_cart) # 0.8112266
tableTPR(test$Top100, predict_cart) # 0.5669291
tableFPR(test$Top100, predict_cart) # 0.1011299
...
###### Model 3.5 10-CV CART Model Optimized for ROC #####
set.seed(123)
...
tableAccuracy(test$Top100, predict_cart_ROC) # 0.7941788
tableTPR(test$Top100, predict_cart_ROC) # 0.5354331
tableFPR(test$Top100, predict_cart_ROC) # 0.1129944
...
##### Model 5: Bagging ##### (Takes about 15 min to run)
set.seed(123)

bag_model <- randomForest(x = train.mm, y = train$Top100, mtry = 83, nodesize = 5, ntree = 500)
...
##### Model 6: 10-CV Boosting Optimized for ROC ##### 
set.seed(123)
...
boostPerformance <- performance(rocr.boost.pred, "tpr", "fpr")
plot(boostPerformance, colorize = TRUE)
abline(0, 1)

as.numeric(performance(rocr.boost.pred, "auc")@y.values) # AUC = 0.8902109
...
##### Model 8: 10-CV SVM ##### (NEED TO RUN)
set.seed(123)

objControl <- trainControl(method="cv", number=10, classProbs=TRUE, summaryFunction=twoClassSummary)

train_svm <- train(Top100 ~ .,
                   data = train,
                   method = "svmRadial",
                   trControl = objControl, 
                   tuneLength = 20)

svmPredict <- predict(train_svm, newdata = test)
table(test$Top100, svmPredict)
tableAccuracy(test$Top100, svmPredict) # ?
tableTPR(test$Top100, svmPredict) # ?
tableFPR(test$Top100, svmPredict) # ?
